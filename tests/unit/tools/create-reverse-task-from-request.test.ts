...
describe('create_reverse_task_from_request tool', () => {
  it('creates a reverse task from one captured request', async () => {
    const rootDir = await mkdtemp(path.join(tmpdir(), 'jsreverser-task-from-request-'));
    ...
    try {
      ...
      }, response as unknown as Parameters<typeof createReverseTaskFromRequestTool.handler>[1], {
        getNetworkRequestById: (requestId: number) => {
          assert.strictEqual(requestId, 7);
          return {
            url: () => 'https://example.com/api/sign',
            method: () => 'POST',
            headers: () => ({'content-type': 'application/json', 'x-sign': '1'}),
            postData: () => '{"token":"abc"}',
            resourceType: () => 'fetch',
      ...
      } as unknown as Parameters<typeof createReverseTaskFromRequestTool.handler>[2]);
      ...
      const targetContext = JSON.parse(
        await readFile(path.join(rootDir, payload.taskId!, 'target-context.json'), 'utf8'),
      ...
      assert.strictEqual(((targetContext.targetRequest as Record<string, unknown>).method), 'POST');
      assert.strictEqual(((targetContext.targetRequest as Record<string, unknown>).url), 'https://example.com/api/sign');
    } finally {
