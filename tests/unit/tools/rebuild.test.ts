...
import {diffEnvRequirements, exportPortableBundle, exportRebuildBundle, getRebuildHealthReport} from '../../../src/tools/rebuild.js';
...
describe('rebuild bridge tools', () => {
  ...
  it('auto-generates replay.js when rebuild bundle export enables portable replay', async () => {
    const rootDir = await mkdtemp(path.join(tmpdir(), 'jsreverser-mcp-rebuild-auto-portable-'));
    const runtime = getJSHookRuntime();
    const originalStore = runtime.reverseTaskStore;
    const originals = {
      getTopPriorityFiles: runtime.collector.getTopPriorityFiles,
    ...
    try {
      ...
      const replay = await readFile(path.join(rootDir, 'task-002-portable', 'env', 'replay.js'), 'utf8');
      const state = JSON.parse(await readFile(path.join(rootDir, 'task-002-portable', 'state.json'), 'utf8')) as Record<string, unknown>;
      assert.ok(replay.includes('Portable rebuild replay generated by export_portable_bundle'));
  ...
  it('collapses rebuild and pure artifacts into portable single-file outputs', async () => {
    const rootDir = await mkdtemp(path.join(tmpdir(), 'jsreverser-mcp-portable-export-'));
    const runtime = getJSHookRuntime();
    const originalStore = runtime.reverseTaskStore;
    runtime.reverseTaskStore = new ReverseTaskStore({rootDir});
    ...
    try {
      ...
      } as Parameters<typeof exportRebuildBundle.handler>[0], makeResponse() as unknown as Parameters<typeof exportRebuildBundle.handler>[1], {} as Parameters<typeof exportRebuildBundle.handler>[2]);

      await mkdir(path.join(rootDir, 'task-portable-001', 'run'), {recursive: true});
      await writeFile(path.join(rootDir, 'task-portable-001', 'run', 'pure-main.js'), [
      ...
      await writeFile(path.join(rootDir, 'task-portable-001', 'run', 'fixtures.json'), `${JSON.stringify({
        goalMode: 'pure-draft',
        mainFunction: 'signPayload',
        samples: [{caseId: 'fixture-001', input: {token: 'a'}, runtimeContext: {}}],
      }, null, 2)}\n`);
      await writeFile(path.join(rootDir, 'task-portable-001', 'pure-extraction.json'), `${JSON.stringify({
      ...
      } as Parameters<typeof exportPortableBundle.handler>[0], response as unknown as Parameters<typeof exportPortableBundle.handler>[1], {} as Parameters<typeof exportPortableBundle.handler>[2]);

      const payload = extractFirstJsonBlock(response.lines);
      assert.strictEqual(payload.ok, true);
      assert.ok((payload.generatedFiles as string[]).includes('run/portable.js'));
      assert.ok((payload.generatedFiles as string[]).includes('env/replay.js'));

      const portable = await readFile(path.join(rootDir, 'task-portable-001', 'run', 'portable.js'), 'utf8');
      const replay = await readFile(path.join(rootDir, 'task-portable-001', 'env', 'replay.js'), 'utf8');
      const state = JSON.parse(await readFile(path.join(rootDir, 'task-portable-001', 'state.json'), 'utf8')) as Record<string, unknown>;
      const timeline = (await readFile(path.join(rootDir, 'task-portable-001', 'timeline.jsonl'), 'utf8'))
        .trim()
        .split('\n')
      ...
      assert.ok(String(state.currentSummary).includes('env/replay.js'));
      assert.ok(timeline.some((entry) => entry.action === 'compact_delivery_ready'));
    } finally {
      runtime.reverseTaskStore = originalStore;
      await rm(rootDir, {recursive: true, force: true});
    }
