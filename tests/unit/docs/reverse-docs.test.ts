...
describe('reverse workflow docs', () => {
  it('documents task artifacts, local rebuild, and Codex workflow guidance', async () => {
    ...
    const quickReferenceGuide = await readRepoFile('docs/guides/mcp-agent-quick-reference.md');
    const autoResumeGuide = await readRepoFile('docs/guides/mcp-client-auto-resume-example.md');
    ...
    const reverseAgentResponseSchema = await readRepoFile('docs/reference/reverse-agent-response.schema.json');
    const manageResponseSchema = await readRepoFile('docs/reference/manage-response.schema.json');
    const orchestrateResponseSchema = await readRepoFile('docs/reference/orchestrate-response.schema.json');
    const rebuildHealthResponseSchema = await readRepoFile('docs/reference/rebuild-health-response.schema.json');
    const reverseAgentSchemaVersioning = await readRepoFile('docs/reference/reverse-agent-schema-versioning.md');
    ...
    assert.ok(orchestrationGuide.includes('"requiredParams": ["runtimeError", "observedCapabilities"]'));
    ...
    assert.ok(orchestrationGuide.includes('if outcome == blocked: stop and surface blockedBy'));
    ...
    assert.ok(orchestrationGuide.includes('拿到 `outcome=blocked` 还继续盲目 `resume`'));
    assert.ok(orchestrationGuide.includes('把 `recommend_next_step` 当成持久化编排器'));
    assert.ok(orchestrationGuide.includes('忽略 `continuation.ready=false` 还强行执行 `continuation.invoke`'));
    ...
    assert.ok(toolReference.includes('Compact response example (`manage_reverse_task:get`)'));
    assert.ok(toolReference.includes('"requiredParams": ["taskId"]'));
    assert.ok(toolReference.includes('"requiredParams": ["runtimeError", "observedCapabilities"]'));
    assert.ok(toolReference.includes('Failure response example (`env_error`, resumable)'));
    ...
    assert.ok(quickReferenceGuide.includes('如果上下文再紧一点，可以继续压缩成'));
    assert.ok(quickReferenceGuide.includes('mcp-client-auto-resume-example.md'));
    assert.ok(quickReferenceGuide.includes('reverse-agent-response.schema.json'));
    ...
    assert.ok(quickReferenceGuide.includes('rebuild-health-response.schema.json'));
    ...
    assert.ok(reverseAgentResponseSchema.includes('"title": "JSReverser MCP Reverse Agent Response"'));
    ...
    assert.ok(manageResponseSchema.includes('"title": "JSReverser MCP manage_reverse_task Response"'));
    assert.ok(manageResponseSchema.includes('"x-schemaVersion": "1.0"'));
    assert.ok(manageResponseSchema.includes('"action"'));
    assert.ok(orchestrateResponseSchema.includes('"title": "JSReverser MCP orchestrate_reverse_task Response"'));
    ...
    assert.ok(rebuildHealthResponseSchema.includes('"title": "JSReverser MCP get_rebuild_health_report Response"'));
    ...
    assert.ok(reverseAgentSchemaVersioning.includes('Unsupported reverse-agent schema version'));
  });
});
