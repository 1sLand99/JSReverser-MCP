...
function slugifyRequestTarget(url: string): string {
  try {
    ...
    const pathname = parsed.pathname.replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-+|-+$/g, '');
...
export const createReverseTaskFromRequestTool = defineTool({
  ...
  handler: async (request, response, context) => {
    const runtime = getJSHookRuntime();
    const httpRequest = context.getNetworkRequestById(request.params.requestId, request.params.targetPageIdx);
    ...
    ].filter((value, index, items) => items.indexOf(value) === index).slice(0, 5);

    const taskId = request.params.taskId ?? buildTaskId(request.params.requestId, url);
      ...
      targetContext: {
        ...
        targetRequest: {
          ...
          notes: [
            ...
            headerKeys.length > 0 ? `headers=${headerKeys.join(',')}` : undefined,
    ...
    response.appendResponseLine(JSON.stringify({
      ...
      candidateScripts,
      nextRecommendedTools: ['manage_reverse_task', 'get_request_initiator', 'record_reverse_evidence', 'orchestrate_reverse_task'],
    }, null, 2));
    response.appendResponseLine('```');
  },
});
