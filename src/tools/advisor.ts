...
type RecommendNextStepParams = {
  taskId?: string;
  browserHealthy?: boolean;
  pageReady?: boolean;
  taskGoal?: string;
  currentStage?: 'Observe' | 'Capture' | 'Rebuild' | 'Patch' | 'DeepDive' | 'PureExtraction' | 'Port';
  taskStatus?: string;
...
async function buildTaskAwareAdvice(params: RecommendNextStepParams) {
  ...
  const [taskState, taskDescriptor, runtimeEvidence] = await Promise.all([
    runtime.reverseTaskStore.readSnapshot<ReverseTaskState>(params.taskId, 'state.json'),
    runtime.reverseTaskStore.readSnapshot<Record<string, unknown>>(params.taskId, 'task.json'),
  ...
  const hasTargetRequest =
    ...
    (
      Boolean((taskDescriptor?.targetContext as Record<string, unknown> | undefined)?.targetRequest) ||
      ['Capture', 'Rebuild', 'Patch', 'DeepDive', 'PureExtraction', 'Port'].includes(
  ...
  const hookRecordCount =
    ...
    runtimeEvidence.filter((entry) => entry.kind === 'hook-hit' || entry.source === 'hook').length;
  const successCriteria = (taskState?.successCriteria ?? taskDescriptor?.successCriteria ?? {}) as Record<string, unknown>;
  const hasRebuildBundle = params.hasRebuildBundle ??
    (
      ['Rebuild', 'Patch', 'PureExtraction', 'Port'].includes(String(taskState?.currentStage ?? taskDescriptor?.currentStage ?? '')) ||
      ['pass', 'partial'].includes(String(successCriteria.localRebuild ?? ''))
    );
  const localRebuild = successCriteria.localRebuild;
  const currentStage = params.currentStage ?? taskState?.currentStage ?? String(taskDescriptor?.currentStage ?? 'Observe');
...
export const recommendNextStepTool = defineTool({
  ...
  handler: async (request, response) => {
    ...
    response.appendResponseLine(JSON.stringify(await buildTaskAwareAdvice(request.params), null, 2));
...
export const explainReverseStage = defineTool({
  ...
  annotations: {category: ToolCategory.REVERSE_ENGINEERING, readOnlyHint: true},
  schema: {
    stage: zod.enum(['Observe', 'Capture', 'Rebuild', 'Patch', 'DeepDive', 'PureExtraction', 'Port']),
