...
import type {ReverseTaskExecutableStep, ReverseTaskExecutionOverride} from './ReverseTaskExecutor.js';
...
export type ExecutionAdapter = (context: ExecutionAdapterContext) => Promise<ExecutionAdapterResult>;

function makeToolResponse(): {lines: string[]; appendResponseLine(value: string): void} {
  const lines: string[] = [];
  return {
    lines,
...
async function readTaskDescriptor(store: ReverseTaskStore, taskId: string): Promise<Record<string, unknown>> {
  return (await store.readSnapshot<Record<string, unknown>>(taskId, 'task.json')) ?? {};
}

async function inferRuntimeError(store: ReverseTaskStore, taskId: string): Promise<string> {
  ...
  const lastError = [...timeline].reverse().find((entry) => String(entry.status ?? '') === 'error');
  if (typeof lastError?.result === 'string' && lastError.result.trim().length > 0) {
    return lastError.result;
  }
  if (typeof state?.currentSummary === 'string' && state.currentSummary.trim().length > 0) {
...
const manageReverseTaskAdapter: ExecutionAdapter = async ({store, taskId, currentStage, step}) => {
  ...
  if (action === 'timeline') {
    await appendReverseTimeline(store, {
      taskId,
      taskSlug: typeof step.params.taskSlug === 'string' ? step.params.taskSlug : undefined,
      targetUrl: typeof step.params.targetUrl === 'string' ? step.params.targetUrl : undefined,
      ...
      result: typeof step.params.result === 'string' ? step.params.result : 'orchestrated timeline sync',
      next: typeof step.params.next === 'string' ? step.params.next : 'manage_reverse_task:summarize',
    });
...
const overrideAdapter: ExecutionAdapter = async ({store, taskId, currentStage, step, override}) => {
  await appendReverseTimeline(store, {
    taskId,
...
const exportRebuildBundleAdapter: ExecutionAdapter = async ({store, taskId, currentStage}) => {
  ...
  await syncTaskState(store, taskId, currentStage, 'active', '自动编排已导出本地 rebuild bundle');
  return {result: 'rebuild bundle exported'};
};

const diffEnvRequirementsAdapter: ExecutionAdapter = async ({store, taskId, currentStage}) => {
  const runtimeError = await inferRuntimeError(store, taskId);
  const response = makeToolResponse();
  await diffEnvRequirements.handler({
    params: {
      runtimeError,
      observedCapabilities: ['window', 'document', 'navigator', 'localStorage', 'sessionStorage', 'crypto'],
    },
  } as Parameters<typeof diffEnvRequirements.handler>[0], response as unknown as Parameters<typeof diffEnvRequirements.handler>[1], {} as Parameters<typeof diffEnvRequirements.handler>[2]);
  const payload = extractToolJson(response.lines);
  ...
  await syncTaskState(store, taskId, currentStage, 'partial', `自动编排已分析补环境缺口：${missingCapabilities}`);
...
export async function executeRegisteredStep(context: ExecutionAdapterContext): Promise<ExecutionAdapterResult> {
  ...
  if (!adapter) {
    throw new Error(`Automatic execution for tool "${context.step.tool}" is not implemented yet`);
  }
