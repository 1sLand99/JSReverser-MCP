...
async function readTaskDescriptor(store: ReverseTaskStore, taskId: string): Promise<ReverseTaskDescriptor> {
  const task = await store.readSnapshot<ReverseTaskDescriptor>(taskId, 'task.json');
  if (!task) {
...
async function writeTaskDescriptor(store: ReverseTaskStore, descriptor: ReverseTaskDescriptor): Promise<ReverseTaskDescriptor> {
...
export async function archiveReverseTask(store: ReverseTaskStore, taskId: string): Promise<{taskId: string; archivedAt: number}> {
...
export async function restoreReverseTask(store: ReverseTaskStore, taskId: string): Promise<{taskId: string; restored: boolean}> {
  const descriptor = await readTaskDescriptor(store, taskId);
  await writeTaskDescriptor(store, {
    ...descriptor,
...
export async function searchReverseTasks(
  ...
  return rows.filter((row) => {
    ...
    const matchesTag = !normalizedTag || row.tags.some((tag) => tag.toLowerCase() === normalizedTag);
...
export async function pruneReverseTasks(
  ...
  const cutoff = options.olderThanDays && options.olderThanDays > 0
