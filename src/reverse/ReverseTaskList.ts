...
export async function listReverseTasks(
  ...
  const rows = await Promise.all(taskIds.map(async (taskId) => {
    const state = await getReverseTaskState(store, taskId, {timelineLimit: 1, evidenceLimit: 1});
    return {
      taskId,
      slug: String(state.task?.slug ?? ''),
      currentStage: String(state.state?.currentStage ?? state.task?.currentStage ?? 'Observe'),
